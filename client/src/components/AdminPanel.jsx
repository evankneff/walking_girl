...
function AdminPanel() {
  ...
  const fetchUserEntries = async (userName) => {
    ...
    try {
      const res = await fetch(`/api/admin/entries?user_name=${encodeURIComponent(userName)}`, {
        headers: authHeaders,
      });
        ...
        <div className="modal-overlay">
          <div className="modal-card">
            ...
            <p className="modal-warning">
              This will permanently remove <strong>{deleteConfirmUser.name}</strong> and all their entries. This cannot be undone.
            </p>
            <div className="form-group">
            ...
            <div className="modal-actions">
              <button
                ...
                disabled={deleteConfirmInput !== deleteConfirmUser.name || loading}
              ...
              <button
                ...
                onClick={() => { setDeleteConfirmUser(null); setDeleteConfirmInput(''); }}
              >
                Cancel
              </button>
            </div>
      ...
      {selectedUser && activeTab === 'management' && (
        <section className="admin-section per-kid-section">
          <div className="per-kid-header">
            ...
            <button className="cancel-button" onClick={() => { setSelectedUser(null); setUserEntries([]); }}>
              Close
            </button>
          </div>
          <div className="per-kid-summary">
            <span><strong>Total entries:</strong> {userEntries.length}</span>
            <span><strong>Total minutes:</strong> {userTotalMinutes}</span>
          </div>
          {userEntriesLoading ? (
            ...
            <table className="entries-table">
              <thead>
                <tr>
                  ...
                  <th className="sortable" onClick={() => handleSort('minutes')}>
